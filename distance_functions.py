...
def points_in_bounding_box(bound1: Point, bound2: Point, points: set[Point]) -> set[Point]:
    b1x, b1y = bound1
    b2x, b2y = bound2
    ...
    points_in_box: set[Point] = set()
 
    for point in points:
        px, py = point
        if minx <= px <= maxx and miny <= py <= maxy:
            points_in_box.add(point)
 
    return points_in_box
 
def intersect_circles(circle1: Circle, circle2: Circle) -> set[Point]:
...
def intersect_lines(line1: Line, line2: Line) -> set[Point]:
    ...
    intersects = points_in_bounding_box((x1, y1), (x2, y2), {(intersect_x, intersect_y)})
    return points_in_bounding_box((x3, y3), (x4, y4), intersects)
 
def intersect_line_circle(line: Line, circle: Circle) -> set[Point]:
    ...
    return points_in_bounding_box((x1, y1), (x2, y2), intersections)
 
def nearest_point(points: set[Point], point) -> Point | None:
    closest_point = None
    closest_distance = float('inf')
    for p1 in points:
        current_distance = dist(point, p1)
        if current_distance < closest_distance:
            closest_point = p1
            closest_distance = current_distance
    return closest_point
 
def convert_geo_point_to_screen_coords(point: Point, scale):
    x = point[0] * scale + screen_width / 2
    y = point[1] * scale + screen_height / 2
    return x, y
 
def convert_screen_coords_to_geo_point(point, scale):
